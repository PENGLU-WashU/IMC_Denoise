...
- Please go to https://zenodo.org/record/6513175#.YnC2-JLMLpA and download **Raw_IMC_dataset_for_training_supp_table5.zip**. Then unzip this file as the folder **Raw_IMC_dataset_for_training_supp_table5**. This folder contains all the images for DeepSNF training. 

- We also provide all the images of this human bone marrow IMC dataset, which are compressed in **Raw_IMC_dataset_all_supp_table5** and can also be downloaded from https://zenodo.org/record/6513175#.YnC2-JLMLpA. 
...
- Generating training set and train a DeepSNF model.
  ...
  ```
  python scripts/Data_generation_DeepSNF_script.py --channel_name '141Pr' --Raw_directory 'Your_raw_img_directory'  --n_neighbours '4' --n_iter '3' --slide_window_size '3' --ratio_thresh '0.8'
  ```
  ...
  ```
  python scripts/Training_DeepSNF_script.py --train_set_name 'training_set_141Pr.npz' --weights_name 'weights_141Pr-CD38.hdf5' --train_epoches '200' --train_batch_size '64' --lambda_HF '3e-6'
  ```
...
- Implement IMC_Denoise to enhance IMC images.
  ...
  ```
  python scripts/Predict_DIMR_batch.py --channel_name 'the_channel_to_be_denoised' --load_directory 'raw_image_folders (please refer to Section: Directory structure of IMC_Denoise)' --save_directory 'DIMR_processed_image_folders' --n_neighbours '4' --n_iter '3' --slide_window_size '3'
  ```
